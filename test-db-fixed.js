// Fixed database test using correct schema
// Run with: node test-db-fixed.js
...
async function testDatabase() {
  ...
  try {
    ...
    // Test 3: Get course details with correct fields
    if (courseCount > 0) {
    ...
    // Test 8: Get published courses specifically (what discover API should show)
    console.log('\n8. Getting published courses (what should appear in Discover tab)...');
    ...
    publishedCourses.forEach((course, index) => {
      console.log(`   ${index + 1}. ${course.title} - ${course.category || 'No category'} - ${course.level || 'No level'}`);
    });
