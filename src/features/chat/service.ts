...
export async function ensureCourseChatContext(input: {
  ...
  // Check enrollment only if we have a valid user ID (not anonymous)
  let enrollment = null;
  ...
  // Allow access to chat if course is approved, even without enrollment
  // This enables open discussion for approved courses
  if (!enrollment && userId !== "anonymous") {
    console.log("User not enrolled, but allowing chat access for approved course");
    // Don't return null - continue with chat setup
  }
  ...
  // Only create chat membership for authenticated users
  if (userId && userId !== "anonymous") {
