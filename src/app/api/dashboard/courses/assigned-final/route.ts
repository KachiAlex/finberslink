...
/**
 * GET /api/dashboard/courses/assigned-final
 * Shows courses assigned to the student (mock data version)
 */
export const GET = async (request: NextRequest) => {
  try {
    console.log("=== ASSIGNED FINAL API ===");
    
    // Mock assigned courses based on what should be assigned to you
    ...
    return NextResponse.json({
      ...
      counts: {
        ...
        filtered: assignedCourses.length,
      },
      pagination: {
        page: 1,
        pageSize: 12,
        total: assignedCourses.length,
        totalPages: Math.ceil(assignedCourses.length / 12),
      },
      debug: {
        message: "Assigned API - Courses assigned to student",
        coursesFound: assignedCourses.length,
        enrollmentStatus: "ASSIGNED",
        dataSource: "mock (based on expected assignments)",
        timestamp: new Date().toISOString(),
        note: "These are courses that should be assigned to you as a student"
      }
    });
  } catch (error) {
    console.error("Assigned final API error:", error);
    return NextResponse.json(
      { 
