...
async function updateResumeAction(formData: FormData) {
  ...
  // Return success for client-side notification
...
// Update headshot server action
async function updateHeadshotAction(formData: FormData) {
  "use server";
  ...
  await updateResume(slug, { headshotUrl: headshotUrl || null });
  await invalidateDashboardInsights(user.sub);
  revalidatePath(`/resume/${slug}/edit`);
  revalidatePath(`/resume/${slug}`);

  return { success: true, message: "Headshot updated successfully!" };
}
            ...
            <Card className="border border-slate-200/70 bg-white/95">
              ...
              <CardContent>
                <ResumeEditForm
                  ...
                  initialTitle={(resume as any).title}
                  initialPersonaName={(resume as any).personaName ?? ""}
                  initialSummary={(resume as any).summary ?? ""}
                />
              </CardContent>
            </Card>
