...
async function parsePdfFile(file: File): Promise<string> {
  try {
    // Dynamically import pdfjs-dist (use legacy build when available)
    let pdfjsLib: any;
    try {
    ...
    // Disable worker in-browser to avoid dynamic worker fetch (more reliable across environments)
